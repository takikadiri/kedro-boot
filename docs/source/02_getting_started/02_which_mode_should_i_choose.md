...
- If you want to **change the behaviour of ``kedro run`` CLI command** (e.g. to loop over the session multiple times), see the [embedded mode](../05_embedded_mode/01_getting_started_embedded_mode.md) and the concept of ``KedroBootApp`` (which is powered by the ``KedroBootSession``, so read the [standalone mode](../04_standalone_mode/01_getting_started_standalone_mode.md) section before).
- if you want to **serve a kedro pipeline as an API**, see the [KedroBootFastapi tutorial](../06_fastapi_pipeline_serving/01_getting_started_kedro_fast_api_app.md) (which is a specific type of ``KedroBootApp``, so read the the [embedded mode](../05_embedded_mode/01_getting_started_embedded_mode.md) section before)
